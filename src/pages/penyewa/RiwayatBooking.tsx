...
const STATUS_TABS: { label: string; value: BookingStatus | 'semua' }[] = [
  ...
  { label: 'Lainnya',  value: 'dibatalkan'          },
]
...
const RiwayatBooking = memo(() => {
  ...
  const filtered = tab === 'semua'
    ? bookings
    : tab === 'dibatalkan'
      ? bookings.filter(b => b.status === 'dibatalkan' || b.status === 'hangus')
      : bookings.filter(b => b.status === tab)

  return (
    <main className="max-w-lg mx-auto px-4 py-6">
      <h1 className="text-xl font-bold text-slate-900 mb-4">Riwayat Booking</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="font-semibold text-slate-500 text-sm">Belum ada booking</p>
          <p className="text-xs text-slate-300 mt-1">Booking kamu akan muncul di sini</p>
          <button
            onClick={() => navigate('/listing')}
            className="mt-4 px-4 py-2 bg-amber-400 text-slate-900 text-sm font-semibold rounded-xl hover:bg-amber-500 transition-colors"
          >
            Cari Kost
          </button>
        </div>
          ...
          {filtered.map(b => (
            ...
            >
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-slate-900 text-sm truncate mb-1">{b.listingNama}</p>
                <p className="text-xs text-slate-400 truncate mb-2">{b.listingAlamat}</p>
